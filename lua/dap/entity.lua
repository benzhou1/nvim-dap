...
local function set_variable(_, item, _, context)
  ...
  if not parent then
    utils.notify(
      string.format("Cannot set variable on %s, couldn't find its parent container", item.name)
    )
    return
  end
...
function frames.render_item(frame)
  ...
  if session and frame.id == (session.current_frame or {}).id then
    -- Make sure cursor is on the current frame
    vim.api.nvim_win_set_cursor(0, { frame.i, 0 })
